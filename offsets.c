...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/24 17:17:17 by dgajowni          #+#    #+#             */
/*   Updated: 2025/11/24 17:17:18 by dgajowni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	centering_offset_y(t_win_prop **win_prop, int height)
{
	...
	offset_y = ((*win_prop)->height - height * (*win_prop)->scale);
	return (offset_y / 2);
}
...
int	centering_offset_x(t_win_prop **win_prop, int width)
{
	...
	offset_x = ((*win_prop)->width - width * (*win_prop)->scale);
	return (offset_x / 2);
}

int	perspective_offset_x(t_win_prop **win_prop,
	int out_x, int out_y, int height)
{
	int	offset_x;

	offset_x = sqrt(out_x * out_x) * (height - out_y * 3)
		/ (*win_prop)->height / 2;
	if (out_x < 0)
		return (-offset_x);
	return (offset_x);
